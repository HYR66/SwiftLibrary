...
public class editInfo extends Application {
	...
	public void start(Stage primaryStage) {
		...
		editStage.setX(primaryScreenBounds.getMinX());
		editStage.setY(primaryScreenBounds.getMinY());
		editStage.setWidth(primaryScreenBounds.getWidth());
		editStage.setHeight(primaryScreenBounds.getHeight());
		...
		editBt.setStyle("-fx-background-color: #00CED1;-fx-opacity: 0.5;");
		...
		try {
			...
			while (input.hasNextLine()) {
				HTMLS += input.nextLine();
			}
			HTMLS = utils.AESCode.getOrgWord(HTMLS);
			input.close();
		} catch (Exception ex) {
			utils.systemError obj = new utils.systemError("PMCEMSFR001");
			obj.start(null);
			editStage.close();
		}

		Label titleLT = new Label("蜂鸟公告");
		titleLT.setStyle("-fx-font-size:20px;-fx-font-family:KaiTi;");
		HTMLEditor htmlEditor = new HTMLEditor();
		htmlEditor.setHtmlText(HTMLS);
		Button saveBt = new Button("保存");
		saveBt.setStyle("-fx-background-color: #00BFFF;-fx-opacity: 0.8;");
		saveBt.setTextFill(Color.WHITE);
		DropShadow shadowSave = new DropShadow();
		saveBt.addEventHandler(MouseEvent.MOUSE_ENTERED, (MouseEvent e) -> {
			saveBt.setEffect(shadowSave);
		});
		saveBt.addEventHandler(MouseEvent.MOUSE_EXITED, (MouseEvent e) -> {
			saveBt.setEffect(null);
		});
		detailP.getChildren().addAll(titleLT, htmlEditor, saveBt);
			...
			public void handle(ActionEvent event) {
				try {
					PrintWriter write = new PrintWriter(
							new OutputStreamWriter(new FileOutputStream(info.impPath.announce_txt)));
					write.println(utils.AESCode.getAESWord(htmlEditor.getHtmlText()));
					write.close();
					infoL.setText("保存成功");
				} catch (Exception e) {
					new utils.systemError("PMCEMSFR002").start(null);
					editStage.close();
				}
			}
		});
		...
		closeToolBt.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent event) {
				common.mainClose obj = new common.mainClose(editStage);
				obj.start(null);
			}
		});
		...
		editStage.setTitle("蜂鸟图书馆管理平台");
		editStage.initStyle(StageStyle.TRANSPARENT);
		editStage.getIcons().add(new Image(getClass().getClassLoader().getResourceAsStream(info.impPath.icon_IMG)));
		editStage.show();
