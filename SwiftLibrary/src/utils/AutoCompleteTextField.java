...
public class AutoCompleteTextField {
	...
	private List<String> cacheDataList = new ArrayList<String>() {
		...
		public int indexOf(Object searchContext) {
			...
			for (int i = 0; i < size; i++) {
				...
				String indexInput = new String();
				if (cacheDataList.get(i).contains(searchContext.toString())
						|| input.toString().contains(searchContext.toString())
						|| indexInput.contains(searchContext.toString())) {
	...
	AutoCompleteTextField(TextField textField, List<String> cacheDataList) {
		if (null == textField) {
			throw new RuntimeException("TextField Empty");
		}
